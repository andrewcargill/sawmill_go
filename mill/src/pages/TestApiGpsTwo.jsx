...
import TestApiGpsMap from './TestApiGpsMap';

const TestApiGpsTwo = ({ onSave }) => {
  ...
  const handleSaveLocation = async () => {
    try {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            const limitedLatitude = position.coords.latitude.toFixed(14);
            const limitedLongitude = position.coords.longitude.toFixed(14);
            setLatitude(parseFloat(limitedLatitude));
            setLongitude(parseFloat(limitedLongitude));

            // Save the location with the updated coordinates
            saveLocation(parseFloat(limitedLatitude), parseFloat(limitedLongitude));
          },
          (error) => {
            console.error(error);
          }
